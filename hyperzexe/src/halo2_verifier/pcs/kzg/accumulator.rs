use crate::halo2_verifier::{loader::Loader, util::arithmetic::CurveAffine};
use std::fmt::Debug;
...
///
/// Since in circuit everything are in scalar field, but `Accumulator` might
/// contain base field elements, so we split them into limbs. The const generic
/// `LIMBS` and `BITS` respectively represents how many limbs a base field
/// element are split into and how many bits each limbs could have.
#[derive(Clone, Debug)]
...
mod halo2 {
    ...
    impl<'a, C, PCS, EccChip, ScalarChip, const LIMBS: usize, const BITS: usize>
        AccumulatorEncoding<C, Rc<Halo2Loader<'a, C, EccChip, ScalarChip>>, PCS>
        for LimbsEncoding<LIMBS, BITS>
    where
        C: CurveAffine,
        PCS: PolynomialCommitmentScheme<
            ...
            Accumulator = KzgAccumulator<C, Rc<Halo2Loader<'a, C, EccChip, ScalarChip>>>,
        >,
        EccChip: LimbsEncodingInstructions<'a, C, LIMBS, BITS>,
    {
        fn from_repr(
            limbs: &[&Scalar<'a, C, EccChip, ScalarChip>],
    ...
    mod halo2_lib {
        ...
        impl<'a, C, const LIMBS: usize, const BITS: usize>
            LimbsEncodingInstructions<'a, C, LIMBS, BITS> for EccChip<C>
        where
            C: CurveAffineExt,
    ...
    // .iter()
    // .zip_eq(iter::empty().chain(ec_point.x().limbs()).chain(ec_point.y().
    // limbs())) {
