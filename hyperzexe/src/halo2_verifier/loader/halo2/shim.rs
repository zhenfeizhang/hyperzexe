...
mod halo2_lib {
    ...
    use hyperzexe_ecc::{
        ecc::{fixed_base::FixedEcPoint, EcPoint, EccChip},
        fields::{fp::ScalarFieldChip, FieldChip},
    };
    use std::ops::Deref;

    type AssignedScalar<C> = ScalarFieldChip<C>::FieldPoint;
    type AssignedEcPoint<C> = EcPoint<<C as CurveAffine>::Base>;
    ...
    impl<'a, C: PrimeCurveAffine> IntegerInstructions<'a, C::Base> for ScalarFieldChip<C> {
        ...
        C::Base: PrimeField,
    {
        ...
        fn assign_constant(
            ...
            Ok(FixedEcPoint::assign(fixed, ctx))
        }
...
// integer::rns::Common,
// maingate::{
// CombinationOption, CombinationOptionCommon, MainGate, MainGateInstructions,
// RegionCtx, Term,
// },
...
// EccInstructions<'a, C> for BaseFieldEccChip<C, LIMBS, BITS>
// {
...
// .collect_vec();
// let output = match self.mul_batch_1d_horizontal(ctx, pairs.clone(),
// WINDOW_SIZE) { Err(_) => {
...
// });
// });
// self.assert_equal(ctx, lhs,
// rhs).and(eq.then_some(()).ok_or(Error::Synthesis)) }
// }
// }
