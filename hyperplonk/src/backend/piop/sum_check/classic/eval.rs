use crate::{
    backend::{
        piop::sum_check::classic::{
            ClassicSumCheckProver, ClassicSumCheckRoundMessage, ProverState,
        },
        util::{
            arithmetic::{
...
struct GraphEvaluator<F: PrimeField, const IS_ZERO_CHECK: bool> {
    ...
    identitys: Vec<usize>,
    eq_xys: Vec<usize>,
    eq_xy_shifted_00s: Vec<usize>,
    eq_xy_shifted_01s: Vec<usize>,
    eq_xy_shifted_10s: Vec<usize>,
    eq_xy_shifted_11s: Vec<usize>,
    ...
    offsets: (usize, usize, usize, usize, [usize; 4], usize),
...
impl<F: PrimeField, const IS_ZERO_CHECK: bool> GraphEvaluator<F, IS_ZERO_CHECK> {
    fn new(num_vars: usize, challenges: &[F], expression: &Expression<F>, is_sparse: bool) -> Self {
        ...
        ev.offsets.4[0] = ev.offsets.3 + ev.polys.len();
        ev.offsets.4[1] = ev.offsets.4[0] + ev.eq_xy_shifted_00s.len();
        ev.offsets.4[2] = ev.offsets.4[1] + ev.eq_xy_shifted_01s.len();
        ev.offsets.4[3] = ev.offsets.4[2] + ev.eq_xy_shifted_10s.len();
        ev.offsets.5 = ev.offsets.4[3] + ev.eq_xy_shifted_11s.len();
    ...
    fn register_eq_xy_shifted_00(&mut self, idx: usize) -> ValueSource {
        ValueSource::EqXYShifted00(self.register(|ev| &mut ev.eq_xy_shifted_00s, &idx))
    }

    fn register_eq_xy_shifted_01(&mut self, idx: usize) -> ValueSource {
        ValueSource::EqXYShifted01(self.register(|ev| &mut ev.eq_xy_shifted_01s, &idx))
    }

    fn register_eq_xy_shifted_10(&mut self, idx: usize) -> ValueSource {
        ValueSource::EqXYShifted10(self.register(|ev| &mut ev.eq_xy_shifted_10s, &idx))
    }

    fn register_eq_xy_shifted_11(&mut self, idx: usize) -> ValueSource {
        ValueSource::EqXYShifted11(self.register(|ev| &mut ev.eq_xy_shifted_11s, &idx))
    }
    ...
    fn register_expression(&mut self, expr: &Expression<F>) -> ValueSource {
        match expr {
            ...
            Expression::CommonPolynomial(poly) => match poly {
                ...
                CommonPolynomial::EqXYShifted00(idx) => self.register_eq_xy_shifted_00(*idx),
                CommonPolynomial::EqXYShifted01(idx) => self.register_eq_xy_shifted_01(*idx),
                CommonPolynomial::EqXYShifted10(idx) => self.register_eq_xy_shifted_10(*idx),
                CommonPolynomial::EqXYShifted11(idx) => self.register_eq_xy_shifted_11(*idx),
            },
            ...
            Expression::DistributePowers(..) => unreachable!(),
        }
    }
    ...
    fn data(&self, state: &ProverState<F>, start: usize) -> EvaluatorData<F> {
        let mut data = EvaluatorData {
            ...
            eq_xy_shifted_00_steps: vec![F::zero(); self.eq_xy_shifted_00s.len()],
            eq_xy_shifted_01_steps: vec![F::zero(); self.eq_xy_shifted_01s.len()],
            eq_xy_shifted_10_steps: vec![F::zero(); self.eq_xy_shifted_10s.len()],
            eq_xy_shifted_11_steps: vec![F::zero(); self.eq_xy_shifted_11s.len()],
            poly_steps: vec![F::zero(); self.polys.len()],
            calculations: vec![F::zero(); self.offsets.5 + self.calculations.len()],
        };
    ...
    fn evaluate_polys_next<const IS_FIRST_ROUND: bool, const IS_FIRST_POINT: bool>(
        ...
        macro_rules! calculation {
            ...
            (eq_xy_shifted_00s) => {
                data.calculations[self.offsets.4[0]..]
            };
            (eq_xy_shifted_01s) => {
                data.calculations[self.offsets.4[1]..]
            };
            (eq_xy_shifted_10s) => {
                data.calculations[self.offsets.4[2]..]
            };
            (eq_xy_shifted_11s) => {
                data.calculations[self.offsets.4[3]..]
            };
        }
        ...
        // In the current stage, we evaluate the polynomials at the point 0, 1, ...,
        // degree + 1. At the first round, we need to rotate the MLE vector.
        if IS_FIRST_POINT {
            ...
            zip_for_each!(
                (
                    calculation!(eq_xy_shifted_00s).iter_mut(),
                    self.eq_xy_shifted_00s.iter()
                ),
                (|(eval, idx)| *eval = state.eq_xy_shifted_00s[*idx][b_1])
            );
            zip_for_each!(
                (data.eq_xy_shifted_00_steps.iter_mut(), self.eq_xys.iter()),
                (|(step, idx)| *step =
                    state.eq_xy_shifted_00s[*idx][b_1] - &state.eq_xy_shifted_00s[*idx][b_0])
            );
            zip_for_each!(
                (
                    calculation!(eq_xy_shifted_01s).iter_mut(),
                    self.eq_xy_shifted_01s.iter()
                ),
                (|(eval, idx)| *eval = state.eq_xy_shifted_01s[*idx][b_1])
            );
            zip_for_each!(
                (data.eq_xy_shifted_01_steps.iter_mut(), self.eq_xys.iter()),
                (|(step, idx)| *step =
                    state.eq_xy_shifted_01s[*idx][b_1] - &state.eq_xy_shifted_01s[*idx][b_0])
            );
            zip_for_each!(
                (
                    calculation!(eq_xy_shifted_10s).iter_mut(),
                    self.eq_xy_shifted_10s.iter()
                ),
                (|(eval, idx)| *eval = state.eq_xy_shifted_10s[*idx][b_1])
            );
            zip_for_each!(
                (data.eq_xy_shifted_10_steps.iter_mut(), self.eq_xys.iter()),
                (|(step, idx)| *step =
                    state.eq_xy_shifted_10s[*idx][b_1] - &state.eq_xy_shifted_10s[*idx][b_0])
            );
            zip_for_each!(
                (
                    calculation!(eq_xy_shifted_11s).iter_mut(),
                    self.eq_xy_shifted_11s.iter()
                ),
                (|(eval, idx)| *eval = state.eq_xy_shifted_11s[*idx][b_1])
            );
            zip_for_each!(
                (data.eq_xy_shifted_11_steps.iter_mut(), self.eq_xys.iter()),
                (|(step, idx)| *step =
                    state.eq_xy_shifted_11s[*idx][b_1] - &state.eq_xy_shifted_11s[*idx][b_0])
            );
            zip_for_each!(
            ...
            zip_for_each!(
                [
                    ...
                    (
                        calculation!(eq_xy_shifted_00s).iter_mut(),
                        &data.eq_xy_shifted_00_steps
                    ),
                    (
                        calculation!(eq_xy_shifted_01s).iter_mut(),
                        &data.eq_xy_shifted_01_steps
                    ),
                    (
                        calculation!(eq_xy_shifted_10s).iter_mut(),
                        &data.eq_xy_shifted_10_steps
                    ),
                    (
                        calculation!(eq_xy_shifted_11s).iter_mut(),
                        &data.eq_xy_shifted_11_steps
                    ),
    ...
    fn evaluate_next<const IS_FIRST_ROUND: bool, const IS_FIRST_POINT: bool>(
        ...
        // Note: here is a change compared to the original code
        // if !cfg!(feature = "sanity-check") && IS_ZERO_CHECK && IS_FIRST_ROUND &&
        // IS_FIRST_POINT {     return;
        // }

        for (calculation, idx) in self.indexed_calculations.iter().zip(self.offsets.5..) {
    ...
    fn evaluate<const IS_FIRST_ROUND: bool>(
        ...
        data: &mut EvaluatorData<F>,
        state: &ProverState<F>,
        b: usize, // the index of the current evaluation vector
    ) {
        // Note: remove the check that evals.len() > 2
...
impl ValueSource {
    fn indexed(&self, offsets: &(usize, usize, usize, usize, [usize; 4], usize)) -> usize {
        match self {
            ...
            ValueSource::EqXYShifted00(idx) => offsets.4[0] + idx,
            ValueSource::EqXYShifted01(idx) => offsets.4[1] + idx,
            ValueSource::EqXYShifted10(idx) => offsets.4[2] + idx,
            ValueSource::EqXYShifted11(idx) => offsets.4[3] + idx,
            ValueSource::Calculation(idx) => offsets.5 + idx,
        }
    }
}
...
struct EvaluatorData<F: PrimeField> {
    // for each rotation, the current b0 and b1.
    bs: Vec<(usize, usize)>,

    // for each polynomial, all evaluations are generated by eval[0] + x * step
    lagrange_steps: Vec<F>,
    identity_step_first: F,
    identity_step: F,
    eq_xy_steps: Vec<F>,
    eq_xy_shifted_00_steps: Vec<F>,
    eq_xy_shifted_01_steps: Vec<F>,
    eq_xy_shifted_10_steps: Vec<F>,
    eq_xy_shifted_11_steps: Vec<F>,
    poly_steps: Vec<F>,

    // intermediate result of the expression tree
    calculations: Vec<F>,
}
