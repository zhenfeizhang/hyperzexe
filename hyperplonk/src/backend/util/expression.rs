use crate::backend::util::Itertools;
use std::{
...
impl<F: Clone> Expression<F> {
    ...
    pub fn eq_xy_shifted_00(idx: usize) -> Self {
        Expression::CommonPolynomial(CommonPolynomial::EqXYShifted00(idx))
    }

    pub fn eq_xy_shifted_01(idx: usize) -> Self {
        Expression::CommonPolynomial(CommonPolynomial::EqXYShifted01(idx))
    }

    pub fn eq_xy_shifted_10(idx: usize) -> Self {
        Expression::CommonPolynomial(CommonPolynomial::EqXYShifted10(idx))
    }

    pub fn eq_xy_shifted_11(idx: usize) -> Self {
        Expression::CommonPolynomial(CommonPolynomial::EqXYShifted11(idx))
    }
        ...
        F: Debug,
    {
        match self {
            ...
            Expression::CommonPolynomial(poly) => match poly {
                ...
                CommonPolynomial::EqXYShifted00(idx) => {
                    write!(writer, "(1 - xn) * eq_shift_even_{idx}")
                },
                CommonPolynomial::EqXYShifted01(idx) => {
                    write!(writer, "(1 - xn) * eq_shift_odd_{idx}")
                },
                CommonPolynomial::EqXYShifted10(idx) => write!(writer, "xn * eq_shift_even_{idx}"),
                CommonPolynomial::EqXYShifted11(idx) => write!(writer, "xn * eq_shift_odd_{idx}"),
            },
