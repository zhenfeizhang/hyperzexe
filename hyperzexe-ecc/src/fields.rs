...
pub struct FieldExtPoint<FieldPoint: Clone + Debug> {
    ...
    // We do not specify the irreducible `F_p` polynomial used to construct `F_q` here - that is
...
pub trait FieldChip<F: PrimeField> {
    ...
    // a type implementing `Field` trait to help with witness generation (for
