...
pub type ScalarFieldChip<C> = FpConfig<<C as CurveAffine>::Base, <C as PrimeCurveAffine>::Scalar>;
...
impl<F: PrimeField, Fp: PrimeField> FpConfig<F, Fp> {
    ...
    pub fn enforce_less_than_p(&self, ctx: &mut Context<F>, a: &CRTInteger<F>) {
            ...
            let lt = match borrow {
                ...
                Some(borrow) => {
                    let plus_borrow =
                        self.range
                            .gate
                            .add(ctx, Constant(*p_limb), Existing(&borrow));
        ...
        self.range
            .gate
            .assert_is_const(ctx, &borrow.unwrap(), F::ONE)
    }
...
impl<F: PrimeField, Fp: PrimeField> FieldChip<F> for FpConfig<F, Fp> {
    ...
    fn load_private(&self, ctx: &mut Context<'_, F>, a: Value<BigInt>) -> CRTInteger<F> {
        ...
        let a_loaded = CRTInteger::construct(
            OverflowInteger::construct(limbs, self.limb_bits),
            a_native,
            a,
        );
    ...
    fn load_constant(&self, ctx: &mut Context<'_, F>, a: BigUint) -> CRTInteger<F> {
        ...
        let a_limbs = self.range.gate().assign_region(
            ctx,
            decompose_biguint::<F>(&a, self.num_limbs, self.limb_bits)
                .into_iter()
                .map(Constant),
            vec![],
        );
    ...
    fn add_constant_no_carry(
        ...
        for (a_limb, c_limb) in a
            .truncation
            .limbs
            .iter()
            .zip(c.truncation.limbs.into_iter())
        {
        ...
        let native = self
            .range
            .gate
            .add(ctx, Existing(&a.native), Constant(c_native));
        let trunc =
    ...
    fn range_check(
        ...
        // range check limbs of `a` are in [0, 2^n) except last limb should be in [0,
        // 2^last_limb_bits)
    ...
    fn is_soft_zero(&self, ctx: &mut Context<F>, a: &CRTInteger<F>) -> AssignedValue<F> {
        ...
        self.gate()
            .and(ctx, Existing(&is_zero), Existing(&range_check))
    }
    ...
    fn is_soft_nonzero(&self, ctx: &mut Context<F>, a: &CRTInteger<F>) -> AssignedValue<F> {
        ...
        self.gate()
            .and(ctx, Existing(&is_nonzero), Existing(&range_check))
    }
    ...
    fn assert_equal(&self, ctx: &mut Context<F>, a: &Self::FieldPoint, b: &Self::FieldPoint) {
        ...
        for (limb_a, limb_b) in a.truncation.limbs.iter().zip(a.truncation.limbs.iter()) {
            self.range
                .gate
                .assert_equal(ctx, Existing(limb_a), Existing(limb_b));
        }
    }
}
